...
source("Data_Sim/SampPregAuxModel.R")
...
x <- sampPregAuxModel(dspDat=analyData, betaDays=betaDays, 
                      betaCovs=betaCovs, phi=1, fwLen=5, verbose=TRUE)

finalData <- rmSuperfluous(baselinePre, cyclePre, dailyPre, x$pregVecPre, x$wVecPre, x$xiVecPre)
invisible( list2env(finalData, envir=.GlobalEnv) )
rm(list=setdiff(ls(), c("baseline","cycle","daily","W","xi")))
...
save(Y, X, id, U, W, xi, file="Data/PracticeDatAuxMod.RData")
